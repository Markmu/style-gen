...
export interface AnalysisTransport {
  vision: (task:Task,asset:typeof assets.$inferSelect)=>ReturnType<ReturnType<typeof getVisionProvider>['analyze']>;
  structure: typeof structureAnalysis;
  query: (task:Task)=>Promise<PredictionEvidence|null>;
}
const transport:AnalysisTransport={
  vision:(task,asset)=>getVisionProvider({ ...resolveVisionModel(),provider:task.provider as 'gemini'|'replicate',providerModelId:task.modelName! }).analyze({imageUrl:asset.fileUrl,mimeType:asset.mimeType,webhookUrl:buildWebhookUrl('analysis',task.id)}),
...
function validate(input:unknown): Record<string,unknown> & {requestKey:string} {
  const body=record(input,['sourceAssetId','assetId','fileUrl','width','height','mimeType','directionId','requestKey','retryOf']);
...
export async function prepareAnalysis(userId:string,input:unknown) {
  const body=validate(input),hash=hashWorkspaceRequest(body,['sourceAssetId','assetId','fileUrl','width','height','mimeType','directionId','requestKey','retryOf']);
  return withWorkspaceTransaction(userId,body.directionId as string??null,async(tx,originalDirection)=>{
    ...
    if(!asset&&body.sourceAssetId)throw new WorkspaceNotFound();
    if(!asset){
      const url=new URL(body.fileUrl as string);
    ...
    const expected=publicBase?`${publicBase.replace(/\/$/,'')}/references/${userId}/${body.assetId}/original.${ext}`:null;
    ...
    if(!asset)[asset]=await tx.insert(assets).values({id:assetId,userId,type:'reference',fileUrl:body.fileUrl as string,width:body.width as number,height:body.height as number,mimeType:body.mimeType as string}).returning();
    ...
    if(body.retryOf){prior=await taskForUser(userId,body.retryOf as string,tx);if(prior.sourceAssetId!==assetId||prior.directionId!==originalDirection?.id||!(prior.status==='failed'||prior.status==='completed'&&prior.errorStage==='llm'))throw new WorkspaceServiceError('ANALYSIS_RETRY_NOT_ALLOWED',409);}
...
export async function structureTask(task:Task,raw:string,deps:AnalysisTransport=transport) {
  ...
  const [asset]=await db.select().from(assets).where(and(eq(assets.id,task.sourceAssetId),eq(assets.userId,task.userId!)));
  try{
    ...
    const structured=await deps.structure(raw,{taskId:task.id,source:'analysis_route',imageUrl:asset.fileUrl,mimeType:asset.mimeType},storedBinding);
...
export async function dispatchAnalysis(task:Task,deps:AnalysisTransport=transport) {
  ...
  const [asset]=await db.select().from(assets).where(and(eq(assets.id,task.sourceAssetId),eq(assets.userId,task.userId!)));
  try{
    const result=await deps.vision(claimed,asset);
