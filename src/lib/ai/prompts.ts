...
export const MULTI_REFERENCE_INSTRUCTIONS = `
The supplied images are an ordered reference set. Analyze every image together and produce one transferable visual recipe. Compare their color, lighting, texture, composition and mood. Distinguish shared style from image-specific subjects and conflicting choices; do not imply every feature appears in every image. Refer to Reference 1, Reference 2 and Reference 3 in the evidence descriptions where applicable. Use the first image for the default subject and aspect ratio when these differ. Do not emit spatial evidence coordinates for a multi-image set: the evidence viewer has no per-image coordinate binding. Preserve uncertainty and explain conflicts instead of inventing agreement.
`;
