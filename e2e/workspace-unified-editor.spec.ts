...
test.describe('workspace 09 unified editor', () => {
  test('shows one editor with variables, full text, and JSON modes after analysis', async ({ page }) => {
    ...
    await expect(page.getByLabel('Prompt mode')).toBeVisible()
    await expect(page.getByLabel('Prompt mode').getByRole('option')).toHaveText([
      /Variables/,
      /Full text/,
      /JSON/,
    ])
