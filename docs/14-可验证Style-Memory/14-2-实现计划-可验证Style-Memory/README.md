...
| 变更记录 | 2026-08-26 | 文案英文化（首批英语区用户）：需求 14 引入的用户可见文案由中文统一改为英文，术语映射——用户已验证→User verified、待验证→Pending verification、查看详情→View details、使用这条 Memory→Use this memory、验证依据→Evidence、保留的风格→Retained style、可替换内容→Replaceable、排除约束与增强方向→Constraints & enhancements、风格指纹→Style fingerprint、增强方向→Enhancement hints、规则待补充→No rules yet、无预览→No preview、N 个变量→N variables、必填→Required、设为代表结果→Set as representative result、保存后状态→After saving、仍会保留→What stays、加载更早→Load earlier、进入工作区→Enter workspace、将保留→What carries over、开始前替换→Replace before you start、其他变量（N 项）→Other variables (N)、已恢复 N 条保留规则→Restored N retained rules、仍需填写 X 项→X fields left to fill；4 个 style-memory e2e spec 的文案断言同步更新，行为断言（URL/请求体/焦点/计数/禁用门/字节级快照对比）零放宽；模板页视觉回归（ai-first-visual-regression 为 DOM 状态断言、无像素基线）随卡片文案同步更新断言。 |
